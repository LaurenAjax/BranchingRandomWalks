...
class Node:
    ...
    def propogate_me(self):
        ...
        # print(self.spread)
        spread = lerp(self.spread / 90 * math.acos(density - 1), math.radians(self.spread), self.alpha)
        # print(spread)
        # print(alpha)
...
rootParams = [[0,0], 0, 0, None, 1, dC, cB, rA, pA, 45, 10]
...
for alpha in range(len(alphaArr)):
    plot.subplot(3, 3, alpha + 1)
    root = Node(*rootParams, alphaArr[alpha])
    root.run_gens(gens)
